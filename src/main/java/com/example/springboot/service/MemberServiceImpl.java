...
import com.example.springboot.security.util.JwtProcessor;
...
public class MemberServiceImpl implements MemberService {
    ...
    public int socialSignup(SocialMemberDTO socialMemberDTO) {
        return memberRepository.insertSocialMember(socialMemberDTO);
    }
    ...
    public List<Map<String, Object>> findIdByNameAndPhone(FindIdRequestDTO findIdRequestDTO) {
        ...
        List<MemberEntity> members = memberRepository.findMemberByNameAndPhone(findIdRequestDTO);
        if (members.isEmpty()) {
            System.out.println("MEMBER 없다");
            throw new NoMatchingMemberException(ExceptionCodeEnum.NO_MATCHING_MEMBER.getMessage());
        }
        ...
        for (MemberEntity member : members) {
            ...
            if (member.getSnsType() == null || member.getSnsType().isEmpty()) {
                result.put("sns", null);
            } else if ("kakao".equals(member.getSnsType())) {
    ...
    public void findPassword(FindPasswordRequestDTO findPasswordRequestDTO) {
        ...
        List<MemberEntity> members = memberRepository.selectMemberByIdAndNameAndPhone(findPasswordRequestDTO);
        if (members.isEmpty()) {
            throw new NoMatchingMemberException(ExceptionCodeEnum.NO_MATCHING_MEMBER.getMessage());
        }
    ...
    public int changePassword(ChangePasswordRequestDTO changePasswordRequestDTO) {
        ...
        if (newPassword.equals(changePasswordRequestDTO.getPasswordConfirmation())) {
            ...
            memberRepository.updatePasswordById(changePasswordRequestDTO.getMemberId(), password);
            return 1;
        }

        if (!changePasswordRequestDTO.getPassword().equals(changePasswordRequestDTO.getPasswordConfirmation())) {
            // TODO 익셉션이랑 결과메시지 변경할 것
            throw new PasswordMismatchException(ExceptionCodeEnum.INVALID_CREDENTIALS.getMessage());
        }

        // DB에서 아이디, 이름, 휴대폰번호로 비밀번호 변경
        return memberRepository.updatePasswordById(changePasswordRequestDTO.getMemberId(), passwordEncoder.encode(changePasswordRequestDTO.getPassword()));
    }
    ...
    public LoginResponseDTO login(LoginRequestDTO loginRequestDTO, HttpServletRequest request, HttpServletResponse response) {
        // 사용자 정보 조회
        MemberEntity member = memberRepository.findByMemberId(loginRequestDTO.getMemberId());
        if (member == null) {
            System.out.println("유저없음");
            throw new InvalidCredentialsException(ExceptionCodeEnum.INVALID_CREDENTIALS.getMessage());
        }
        ...
        if (!passwordEncoder.matches(loginRequestDTO.getPassword(), member.getPassword())) {
            memberRepository.incrementPasswordFailureCount(member.getMemberId());
            System.out.println("비밀번호틀림");
            throw new InvalidCredentialsException(ExceptionCodeEnum.INVALID_CREDENTIALS.getMessage());
        }

        memberRepository.resetPasswordFailureCount(member.getMemberId()); // 비밀번호 입력 횟수 리셋~~~
    ...
    public void logout(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (authentication != null) {
            ...
            if (member != null && CommonCodeEnum.KAKAO.getValue().equals(member.getSnsType())) {
    ...
    private void checkTermsOfUse(MemberDTO memberDTO) {
        List<TermsOfUseEntity> termsOfUseList = termsOfUseRepository.selectListTermsOfUseByRequired(CommonCodeEnum.YES.getValue());
        ...
        if (termsOfUseList == null || termsOfUseList.isEmpty() || memberDTO.getTermsAgreementDTOList() == null || memberDTO.getTermsAgreementDTOList().isEmpty()) {
            throw new ApplicationContextException(ExceptionCodeEnum.NO_EXIST_TERMS_OF_USE.getMessage());
        }

        Set<Long> requiredTermsOfUseIds = termsOfUseList.stream().map(TermsOfUseEntity::getTermsOfUseId).collect(Collectors.toSet());
    ...
    private void checkVerifyCodeForSignup(MemberDTO memberDTO) {
        ...
        if (!isVerified) {
            throw new ApplicationContextException(ExceptionCodeEnum.INVALID_VERIFICATION_CODE.getMessage());
        }
    }
    ...
    private void checkDuplicatedMemberId(String memberId) {
        int memberCount = memberRepository.selectMemberById(memberId);

        if (memberCount > 0) {
            throw new MemberAlreadyExistsException(ExceptionCodeEnum.DUPLICATED_MEMBER_ID.getMessage());
        }
    }
    ...
    private static void checkPassword(MemberDTO memberDTO) {
        if (!memberDTO.getPassword().equals(memberDTO.getPasswordConfirmation())) {
            throw new PasswordMismatchException(ExceptionCodeEnum.PASSWORD_MISMATCH.getMessage());
        }
    }
    ...
    private void checkEmailVerificationCode(String emailVerificationCode) {
        ...
        if (!isVerified) {
            throw new EmailVerificationException(ExceptionCodeEnum.INVALID_EMAIL_VERIFICATION.getMessage());
        }
    }

    // 핸드폰 번호로 회원 찾기
    public MemberEntity findByPhoneNumber(String phoneFirst, String phoneMiddle) {
        return memberRepository.findMemberByPhoneNumber(phoneFirst, phoneMiddle);
    }
    ...
    public LoginResponseDTO renewLogin(String token, HttpServletRequest request, HttpServletResponse response) {
        ...
        if (isValidToken) {
            ...
            if (member == null) {
                throw new MemberIdNotFoundException(ExceptionCodeEnum.NO_EXIST_MEMBER_ID.getMessage());
            }
            ...
            if (CommonCodeEnum.YES.getValue().equals(member.getAutoLogin().toString())) {
                // 인증 객체 생성
                Authentication authentication = new UsernamePasswordAuthenticationToken(
                        member.getMemberId(),
                ...
                throw new SessionExpiredException(ExceptionCodeEnum.SESSION_EXPIRATION.getMessage());
            }
        } else {
            throw new SessionExpiredException(ExceptionCodeEnum.SESSION_EXPIRATION.getMessage());
        }
