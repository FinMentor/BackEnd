...
import com.example.springboot.dto.SocialMemberDTO;
import com.example.springboot.entity.MemberEntity;
import com.example.springboot.exception.SocialOauthException;
...
public class NaverOauthServiceImpl implements NaverOauthService {
    ...
    public LoginResponseDTO processNaverLogin(String code, String state, HttpServletRequest request, HttpServletResponse response) {
        ...
        if (memberInfo != null) {
            ...
            MemberEntity existingMember = memberService.findByPhoneNumber(phoneFirst, phoneMiddle);
            ...
            if (existingMember != null) {
                ...
                if (decryptedPhoneLast.equals(phoneLast)) {
                    if (CommonCodeEnum.NAVER.getValue().equals(existingMember.getSnsType())) {
                        //기존회원
                        memberId = existingMember.getMemberId();
                        isExistingMember = true;
                    } else if (CommonCodeEnum.KAKAO.getValue().equals(existingMember.getSnsType())) {
                        System.out.println("카카오 계정 있움");
                        throw new SocialOauthException(ExceptionCodeEnum.KAKAO_ACCOUNT_ALREADY_EXISTS.getMessage());
                    } else {
                        System.out.println("일반 회원 계정 있음");
                        throw new SocialOauthException(ExceptionCodeEnum.ACCOUNT_ALREADY_EXISTS.getMessage());
                    }
                }
            }
            ...
            if (!isExistingMember) {
                ...
                socialMemberDTO.setSns(CommonCodeEnum.NAVER.getValue());
        ...
        throw new SocialOauthException(ExceptionCodeEnum.NAVER_LOGIN_FAIL.getMessage());
    }
}